 ...
 */
void Adxl375<SPI_COM, IOHANDLER, TIMER, INTERRUPT>::init_interupt(uint8_t interrupt_pin) {
 ...
 */
void Adxl375<SPI_COM, IOHANDLER, TIMER, INTERRUPT>::write(uint8_t addr, uint8_t *data,
                                                          uint8_t size) const {
  const bool multiple_bytes = size > 1;
  spi_->beginTransaction(clock_speed_, spi::BitOrder::MSBFIRST, spi::Mode::SPI_MODE3);
  wire_->write(chip_select_, wire::PinStatus::LOW);
 ...
 */
acc::Data Adxl375<SPI_COM, IOHANDLER, TIMER, INTERRUPT>::data() const {
  acc::RawData raw_data{};
 ...
 */
const acc::RawData *Adxl375<SPI_COM, IOHANDLER, TIMER, INTERRUPT>::raw_data() {
  new_data_ = false;
  return &fifo_[0];
}
template <spi_com SPI_COM, iohandler_concept IOHANDLER, time_handler TIMER,
 ...
 */
void Adxl375<SPI_COM, IOHANDLER, TIMER, INTERRUPT>::isr_read_fifo(void *accel) {
  auto &adxl375_type = *static_cast<Adxl375<SPI_COM, IOHANDLER, TIMER, INTERRUPT> *>(accel);
